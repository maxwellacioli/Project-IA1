...
public class Conjunction extends NonTerminal {
	...
	private void sameTerminal(Parser parser) {
		...
		if (getLeftExpression() instanceof Terminal && getRightExpression() instanceof Terminal) {
			Terminal left = (Terminal) getLeftExpression();
			Terminal right = (Terminal) getRightExpression();
	...
	private void walkPreOrderAST(LogicalExpression node, Terminal terminal, Parser parser) {
		if (node == null) {
			return;
		}
